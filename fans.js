...
const requestWithId = (id, reqOptions, body) => request(reqOptions, body).map(throwIfNotTheRightFan(id));
...
const getFanStatus = (ip, id) => requestWithId(id, `coap://${ip}/control/${id}`);
const getFanWifi = (ip, id) => requestWithId(id, `coap://${ip}/wifi/${id}`);
const getFanDiagnostics = (ip, id) => requestWithId(id, `coap://${ip}/diagnostic/${id}`);
const updateFanName = (ip, id, name) => requestWithId(id, {method: "PUT", hostname: ip, pathname: `/device/${id}`}, {name});
const setTimeRemaining = (ip, id, remaining) => requestWithId(id, {method: "PUT", hostname: ip, pathname: `/control/${id}`}, { remaining });
const setCurrentSpeed = (ip, id, speed) => requestWithId(id, {method: "PUT", hostname: ip, pathname: `/control/${id}`}, {speed});
const updateFanSpeeds = (ip, id, speeds) => requestWithId(id, {hostname: ip, pathname: `/control/${id}`, method: "PUT"}, {sequence: sequences[speeds]});
